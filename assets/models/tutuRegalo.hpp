...
void bindings()
{
	if (ImGui::SliderInt("Seed", &seed, 0, 1000))haveToGenerateModel = true;
	if (ImGui::SliderInt("Resolution", &cylinderRes, 3, 8))haveToGenerateModel = true;
	if (ImGui::SliderInt("Max cylinders", &maxCount, 3, 50))haveToGenerateModel = true;
}
...
void generateModel()
{
	...
	srand(seed);
	Cylinder(0.1, cylinderRes, vec::zero, vec(0, 5, 0));
	...
	for (int counter = 0; counter < maxCount; counter++)
	{
		...
		*/
		...
		Cylinder(0.1, cylinderRes, newPosA, newPosB);
