...
void bindings()
{
	if (ImGui::SliderInt("Steps", &steps, 1, 15))haveToGenerateModel = true;
	...
	if (ImGui::SliderFloat("Fractal offset", &fractalOffset, 0.01, 0.5))haveToGenerateModel = true;
}
...
void generateAt(float r, float l, const config& c)
{
	...
	if (c.o)
	{
		posA = c.p + vec::up * l * 0.5;
		posB = c.p - vec::up * l * 0.5;
		Cylinder(r, cylinderRes, posA, posB, false);
		newConfigs.p = c.p + vec::up * l * fractalOffset; configs.back().push_back(newConfigs);
		newConfigs.p = c.p - vec::up * l * fractalOffset; configs.back().push_back(newConfigs);
	}
	else
	{
		posA = c.p + vec::right * l * 0.5;
		posB = c.p - vec::right * l * 0.5;
		Cylinder(r, cylinderRes, posA, posB, false);
		newConfigs.p = c.p + vec::right * l * fractalOffset; configs.back().push_back(newConfigs);
		newConfigs.p = c.p - vec::right * l * fractalOffset; configs.back().push_back(newConfigs);
	}
}
